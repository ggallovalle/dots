...
class ChezmoiActions:
    ...
    def add_keymaps(self, builder: KeymapsBuilder) -> None:
...
def emit_documented_shortcut(builder: KeymapsBuilder, shortcut_def: DocumentedShortcut) -> None:
...
def kitty_defaults(builder: KeymapsBuilder) -> None:
    ...
    builder.setting("kitty_mod", "ctrl+shift")
    builder.blank()
    ...
    for shortcut_def in DEFAULT_SHORTCUTS:
        ...
        emit_documented_shortcut(builder, shortcut_def)
...
def build_keymaps(*, source_path: Path | None = None) -> KeymapsBuilder:
    builder = KeymapsBuilder(shell=f"{USER_SHELL} -lc")
    kitty_defaults(builder)
    custom_keymaps(builder)
    ChezmoiActions(source_path=source_path).add_keymaps(builder)
    return builder


def main() -> None:
    print(build_keymaps(source_path=HOME / "dots/home"))
