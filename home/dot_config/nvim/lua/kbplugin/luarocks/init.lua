...
function Runtime.wire_paths(luarocks, lua_version, cb)
  ...
  process.execute_text(process.make(path_cmd[1], vim.list_slice(path_cmd, 2)), function(err, path_result)
    if err ~= nil or path_result == nil or path_result.code ~= 0 then
      cb(false, "failed to read luarocks path: " .. vim.trim((path_result and path_result.stderr) or tostring(err or "")))
      return
    end

    process.execute_text(process.make(cpath_cmd[1], vim.list_slice(cpath_cmd, 2)), function(err2, cpath_result)
      if err2 ~= nil or cpath_result == nil or cpath_result.code ~= 0 then
        cb(false, "failed to read luarocks cpath: " .. vim.trim((cpath_result and cpath_result.stderr) or tostring(err2 or "")))
        return
      end
      ...
      for _, p in ipairs(vim.split(vim.trim(path_result.stdout or ""), ";", { trimempty = true })) do
      ...
      for _, p in ipairs(vim.split(vim.trim(cpath_result.stdout or ""), ";", { trimempty = true })) do
...
function Installer.write_temp_rockspec(deps)
  ...
  local lines = {
    'package = "kbplugin-deps"',
    'rockspec_format = "3.0"',
    'version = "1.0-1"',
    "source = {",
    '   url = "file:///tmp/kbplugin-deps.tar.gz",',
    "}",
    "dependencies = {",
    '   "lua >= 5.1, < 5.6",',
  }
