...
local function set_range_text(range, replacement)
  ...
  vim
    .api
    .nvim_buf_set_text(0, range.start_lnum - 1, range.start_col, range.end_lnum
    - 1, range.end_col + 1, lines)
end

local function tz_offset_colon(date_fmt, epoch)
  local today = tostring(os.date(date_fmt, epoch))
  return (string.gsub(today, "([+-]%d%d)(%d%d)$", "%1:%2"))
end
...
local function parse_input(s)
  ...
  do
    local y, mo, d, hh, mm, ss, sign, th, tm = s:match("^(%d%d%d%d)%-(%d%d)%-(%d%d)T(%d%d):(%d%d):(%d%d)([+-])(%d%d):?(%d%d)$")
    if y then
      return {
        kind = "rfc3339",
        year = tonumber(y),
...
local function conversions_for(range)
  ...
  if not parsed then
    return nil,
      "Unsupported time format. Expected HH:MM, HH:MM:SS, or RFC3339 (YYYY-MM-DDTHH:MM:SSZ / ±HH:MM)."
  end
